<template>
    <basic-container>
            ...
            <el-tab-pane label="承包商列表">
                <avue-crud ref="crud" :option="option" :data="list" :page="page" v-model="obj"   @row-update = "addUpdate" >
                    <template slot-scope="scope" slot="menuLeft">
                        ...
                        >
                            <el-button type="success" size="mini" v-if="status == 1 ">上传方案</el-button>
                            <!--                            <el-button type="success" size="mini"  >修改承包商</el-button>-->
                        </el-upload>
                        <el-button  type="success" size="mini" v-if="status == 1 "  @click="openBtn(scope.row)">选择承包商</el-button>
                        <el-button
                                type="warning"
                                @click="handleGets"
                                class="btn"
        ...
        <el-dialog title="选择承包商" :visible.sync="dialogPayVisible" :modal=false >
            <el-form :model="form">
                <el-form-item>
                    <avue-crud ref="avue" :option="options" :data="lists" :page="pages" v-model="obj"  @size-change="sizeChange" @current-change="currentChange"
                               @selection-change="selectionChange" @set-current="setCurrent"></avue-crud>
                </el-form-item>
    ...
    export default {
        ...
        methods: {
            ...
            delBtn(row){
                ...
                obj.sid = row.sid;
                obj.id = row.id;
                obj.tid = this.$route.query.id;
                //obj.status = 0
                 //console.log(obj);return false;
                this.$postRequest(baseUrl+'/admin/task/dele',obj).then(res=>{
                    if(res.data.code == '200'){
            ...
            handleGets() {
                window.open(baseUrl + "/normal/excel/persons?tid="+this.$route.query.id);
            },
            upload_success(res) {
                // console.log(res);
                if (res.code == 200) {
