...
<!--            </template>-->
            <template slot-scope="scope" slot="menu" >
                <el-button  icon="el-icon-view" type="text" size="small"  @click="handleClick(scope.row)">查看方案</el-button>
<!--                <el-button type="text"-->
<!--                           size="mini"-->
<!--                           class="btn"-->
<!--                           icon="el-icon-edit"-->
        ...
        methods:{
                ...
                this.$getRequest(baseUrl+'/admin/task/lists',this.page).then(res=>{
                    if(res.data.code == '200'){
                        ...
                        for(var i in  res.data.data.list){
                            ...
                            this.data[i].tasktime = res.data.data.list[i].starttime.substr(0,10)+'<br/>'+res.data.data.list[i].endtime.substr(0,10)
                        }
            ...
            handleClick(row){
                this.$router.push({
                    ...
                    query:{id:row.tid,status:row.status}
                });
            },
            checkBtn(row){
                var obj={};
                ...
                this.$postRequest(baseUrl+'/admin/task/setStatus',obj).then(res=>{
                    ...
                    console.log(res.data);
                        this.$message.error('审核失败，请重新尝试:'+res.data.data.msg);
                    }
                })
            },
            closeBtn(row){
            ...
            edit(row,index,done,loading){
                ...
                obj.tid = row.tid;
                obj.data.title = row.title;
                obj.data.status = row.status;
                obj.data.starttime = row.starttime;
                obj.data.endtime = row.endtime;
                obj.data.caption = row.caption;
                this.$postRequest(baseUrl + "/admin/task/update", obj).then((res) => {
                    if (res.data.code == "200") {
